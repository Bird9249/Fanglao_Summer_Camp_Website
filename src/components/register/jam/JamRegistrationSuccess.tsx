...
import type { JamRegistrationFormValues } from '~/lib/jam-registration'

type JamRegistrationSuccessProps = {
...
function formatMemberLine(name: string, aka?: string) {
  const trimmedAka = aka?.trim()
  if (trimmedAka) return `${name} (AKA: ${trimmedAka})`
  return name
}

export function JamRegistrationSuccess({
  registrationId,
  data,
  categoryLabel,
  onRegisterAgain,
}: JamRegistrationSuccessProps) {
  const isDuo = !!data.teamName?.trim()
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="jam-reg-success-title"
    >
          ...
          <CardHeader className="px-6 pb-2">
            <div className="flex w-full flex-col items-center text-center">
              ...
              <CardDescription className="mt-2 max-w-md text-center text-base">
                ຂໍຂອບໃຈທີ່ສະໝັກ {jamEventMeta.title} — ທີມງານຈະຕິດຕໍ່ຢືນຢັນການເຂົ້າແຂ່ງຂັນໃນໄວໆນີ້
              </CardDescription>
            </div>
          </CardHeader>
          ...
          <CardContent className="flex flex-col gap-4 text-sm">
            <div className="rounded-xl border border-amber-500/35 bg-amber-500/10 p-4 text-center">
              ...
              <p className="mt-2 text-muted-foreground text-xs leading-relaxed">
                ທີມ Fanglao Studio ຈະໂທຫຼືຕິດຕໍ່ກັບຄືນເພື່ອຢືນຢັນການເຂົ້າແຂ່ງຂັນ — ກະລຸນາເກັບເບີໂທໄວ້ໃຫ້ພ້ອມ
              </p>
            </div>
            ...
            <div className="rounded-xl border border-border/70 bg-muted/30 p-4">
              <p>
                <span className="text-muted-foreground">ໝວດແຂ່ງຂັນ:</span>{' '}
                <span className="font-medium">{categoryLabel}</span>
              </p>
              {isDuo ? (
                <>
                  <p>
                    <span className="text-muted-foreground">ຊື່ທີມ:</span>{' '}
                    <span className="font-medium">{data.teamName}</span>
                  </p>
                  <p>
                    <span className="text-muted-foreground">ສະມາຊິກທີ 1:</span>{' '}
                    <span className="font-medium">
                      {formatMemberLine(data.fullName, data.nickname)}
                    </span>
                  </p>
                  {data.partnerFullName ? (
                    <p>
                      <span className="text-muted-foreground">ສະມາຊິກທີ 2:</span>{' '}
                      <span className="font-medium">
                        {formatMemberLine(
                          data.partnerFullName,
                  ...
                  <p>
                    <span className="text-muted-foreground">ເບີໂຕິດຕໍ່ຫຼັກ:</span>{' '}
                    <span className="font-medium">{data.phone}</span>
                  </p>
                  {data.partnerPhone?.trim() ? (
                    <p>
                      <span className="text-muted-foreground">ເບີສະມາຊິກທີ 2:</span>{' '}
                      <span className="font-medium">{data.partnerPhone}</span>
                    </p>
                  ) : null}
                </>
              ) : (
